...
7. On the sidebar click on *Configuration* -> *Application setttings* -> *Advanced edit* button -> add the following application settings to the json:
    ...
    - EP_HOST_NAME - EP server name.
    
    - UKC_PARTITION - UKC partition name.
    
    - UKC_SERVER_IP - UKC server ip. 
    
    - CLIENT_TEMPLATE_NAME - Ephemeral client template name.
    
    - CLIENT_TEMPLATE_ACTIVATION_CODE - Ephemeral client template activation code.
