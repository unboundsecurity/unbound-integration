...
call offsign_ub_ksp_runner.bat -i ..\signed_files\book_with_macro_signed.xlsm -c %cert% -p test -u sasha -w Password1!
rem %my_signtool% sign /v /t http://timestamp.digicert.com /fd sha256 /n %cert% "..\signed_files\book_with_macro_signed.xlsm"
...
call offsign_ub_ksp_runner.bat -i ..\signed_files\word_doc_with_macros_signed.docm -c %cert% -p test -u sasha -w Password1!
rem %my_signtool% sign /v /t http://timestamp.digicert.com /fd sha256 /n %cert% "..\signed_files\word_doc_with_macros_signed.docm"
